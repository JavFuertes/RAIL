...
| :-- | :-- | :-- | :-- |
| Determining railway component stiffnesses | Rail stiffness [**Kr**] & Ballast stiffness [**Kb**] | Location and magnitude of peak Receptance & frequency | RAIL_target_frf_1.csv<br>RAIL_target_frf_2.csv |
...
def inv_solver(DATA_LAB,Frequency_LAB,Kr_guess,Kb_guess,num_elem_slp,step_size, tolerance):
    ...
    return Kr, Kb
```
Nevertheless, the author acknowledges some lackings in this approach and where the current capabilities of the solver are be insufficient these additions could improve making this solution more robust and precise. Firstly the gradient based method can be significantly improved through having a automatically adjusted step size by calculating the ratio of the current frequency against the prediction frequency, since currently there exists the possibility that a combination of prescribed `tolerance` and `step_size` be such that the algorithm is unable to converge. Moreover, the `TRACK_v3.0` FE module also calculate the frequency response function by also using the number of sleepers as a variable when calling the `black_box_model` therefore the current approach does not incorporate for this variable and may therefore constrain the capability of the solution to reconstruct the response in situations where the mesh influence is big. Although as per the investigation it was demonstrated that this effect is small hence, why it was omitted in the first place.   
