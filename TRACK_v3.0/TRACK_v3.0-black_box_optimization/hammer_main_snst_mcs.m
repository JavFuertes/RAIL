...
function [H1, fxx,dis_x_load] = hammer_main_snst_mcs(inp,mat_trk,geo)
[dis, ~,~, ~, force]=solver_newmark(inp,mat_trk,geo);


X_load=inp.ext_force.x;
if X_load(3)==0
%shape function 1 on rail
shape = form_shape_fun(geo,mat_trk,X_load);
...
sf = inp.ext_force.sf;
[H1,~,~,~,~,fxx] =tran_fun([force(1:inp.solver.n_ts)',dis_x_load(1:inp.solver.n_ts)],w,0,25600,sf);
end
% figure;
% plot(t,dis_x_load);
