...
%%
function [in_data,NNslpf] = get_input_Nielsen_2005(in_data)
%%
%Geometry input

in_data.geo.Ltot_R=13;                  %[m]
in_data.geo.SlpSpc=0.65;           %[m] 
...
in_data.mater(1).Data=[210e9;    %E_R[N/m^2]
                    0.3047619e-4;    %I_R[m^4] EI = 6.4e6 Nm^2
...
in_data.mater(2).Data=[74.6e13; % rigid [N/m^2]
                    1.375e-4;  %[m^4]
                    0.03876; %[m^2] L = 1.29 m 
                    2500; %[kg/m^3] Ms(half) = 125 kg
                    74.6e13/2.34; %E/2.34
                    0.833]; %0.833
...
in_data.mater(3).Data=[120e6;%70e6; %K_Spring_RS [N/m]
...
in_data.mater(4).Data = [140e6/NNslph;  %3e7 K_Spring_SB[N/m]
                         165e3/NNslph];%3.1e4];  %C_Damper_SB[N.s/m]
